...
## Project Overview
`BadSeed` is a high-performance, zero-dependency Python CLI tool built to detect known compromised supply chain libraries on a local machine. It was designed to quickly identify specific targets and high-risk versions of libraries that have been identified in recent supply chain attacks.
...
## Data Source
The tool uses a `config.json` file located in the same directory as the script. This file contains the list of tracked libraries, user scan paths, and system paths.
...
### Running the Tool
```bash
# Basic scan (targets user_paths and system locations)
python3 badseed.py
...
- **Semantic Versioning:** Supports standard semver operators in `config.json` including `==`, `>`, `>=`, `<`, `<=`, `~` (tilde), and `^` (caret).
...
- **Ecosystem Isolation:** Maintain separate library lists for Python and npm to avoid false positives across ecosystems.
